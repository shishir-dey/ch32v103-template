...
volatile char rx_dma_buffer[DMA_BUFFER_SIZE];
volatile char tx_dma_buffer[DMA_BUFFER_SIZE];
volatile uint8_t dma_rx_complete = 0;
volatile uint8_t dma_tx_complete = 1; // Initially ready to transmit
...
void uart_dma_loop(void) {
    ...
    if(current_rx_count != last_rx_count) {
        ...
        for(uint16_t i = last_rx_count; i < current_rx_count; i++) {
            char received_char = rx_dma_buffer[i % DMA_BUFFER_SIZE];
...
REGISTER_APP(uart_dma_setup, uart_dma_loop);